...
void DecodeMessage(std::string message, int &frameNumber, int &frameTotal, std::string &data, int &ackFrame) {
	...
	std::string frameNum = message.substr(0,fnPos);
	frameNumber = atoi(frameNum.c_str());
		
	// Get total frames
	std::size_t ftPos = message.find("|");
	std::string frameTot = message.substr(fnPos + 1, ftPos - fnPos - 1);
	frameTotal = atoi(frameTot.c_str());
